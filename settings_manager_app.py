...
class UserSetting(db.Model):
    ...
    setting_type = db.Column(db.String(50), default='text')  # text, toggle, dropdown, slider
...
def update_setting(key: str, value: str) -> dict:
    """Update an existing setting and log history"""
    key = key.lower()
...
def get_settings_summary() -> str:
    """Return formatted settings summary"""
    settings = UserSetting.query.all()
    if not settings:
